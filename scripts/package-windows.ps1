...
try {
  ...
  Push-Location $app
  try { & npm.cmd install --omit=dev --ignore-scripts --no-audit --no-fund; if ($LASTEXITCODE) { throw "Production dependency install failed" } }
  ...
  Copy-Item (Join-Path $repo "installer\install.cmd"), (Join-Path $repo "installer\install.ps1"), (Join-Path $repo "installer\uninstall.ps1"), (Join-Path $repo "installer\run-client.cmd") $stage
  ...
  $manifest = @{ packageVersion = $version; architecture = "win-x64"; nodeVersion = $nodeRelease.version; nodeArchiveSha256 = $expectedNodeHash; createdAt = [DateTime]::UtcNow.ToString("o"); files = $files }
